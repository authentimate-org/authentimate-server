...
//Read All
export const handleGetAllProjectsByIssuerId = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    const allProjects = await ProjectModel.find({ issuerId: req.issuerId })
      .select("projectName _id stage createdAt")
      .sort({ createdAt: -1 })
      .exec();
...
export const handleGetProjectById = async (
  ...
  try {
    ...
    if (project.issuerId.toString() != req.issuerId) {
      return res
        .status(401)
        .json({ error: "Unauthorized (issuer not matched)" });
    }
...
//Update
export const handleUpdateProjectById = async (
  req: Request,
  res: Response
): Promise<Response> => {
...
//Select Template
export const handleSelectPremadeTemplate = async (
  req: Request,
  res: Response
): Promise<Response> => {
  ...
  try {
    ...
    const premadeTemplate = await PremadeTemplateModel.findById(
      premadeTemplateId
    );
    ...
    if (project.issuerId.toString() != req.issuerId) {
      return res
        .status(401)
        .json({ error: "Unauthorised (issuer not matched)" });
    }
...
//Delete
export const handleDeleteProjectById = async (
  req: Request,
  res: Response
): Promise<Response> => {
  const { projectId } = req.body;
